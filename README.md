...
PocketBrain is an **offline-first AI workspace** for mobile — not a text-only chatbot. Users download open-source / local models for a purpose (write, study, code, documents, images, voice), then work with **text, documents, images, audio, and other supported files on-device**.
...
Capabilities depend on the **installed model**, **device hardware**, and **native runtime**. PocketBrain never invents neural answers, fake images, or fake videos.
...
| Purpose-first Home / Get marketplace | ✅ Complete |
| Universal Chat composer (files / camera / gallery) | ✅ Complete (text extract for TXT/MD/CSV/code/XLSX; binary PDF/DOCX metadata + honesty) |
| Chat → real PPTX/PDF/DOCX/XLSX via Workspace | ✅ Complete when a text/documents model is installed |
...
| End-user UX (1.0.1) | ✅ Complete (5 tabs + Files + Workspace tools) |
...
| --- | --- | --- |
| Text chat / coding / study tasks | `ChatOrchestrator` + `AIService` + InferenceEngine | Multi-turn context; real GGUF via `llama.rn` in **native** builds; Expo Go uses labeled **mock** |
| Attachments (files / gallery / camera) | `UniversalComposer` + `AttachmentService` | Copies into app storage; extracts text where possible |
| Capability routing | `capabilityResolver` | Maps prompt + files → chat / summarize / PPT / PDF / vision / image / video gates |
| Document / presentation generation | Workspace generators + exporters | Real DOCX/PDF/PPTX/XLSX files — not placeholders |
...
| Vision | `VisionService` | Prompt-assisted / gated — **pixels not passed into GGUF yet** |
| Image generation | `ImageGenerationService` | **Refuses fake pixels** until diffusion runtime + model exist |
| Video generation | Task UI only | **NOT SUPPORTED** on current runtime — honest block |
...
| [`package.json`](package.json) | Dependencies, version `1.0.1`, npm scripts |
...
6. Vision: image pixels are **not** fed into llama.rn yet; OCR/system STT/TTS remain the honest on-device media paths
7. Image generation needs a linked diffusion runtime + curated model (no fake images)
8. Video generation is **not supported** on the current device/runtime
9. PDF/DOCX/PPTX **text extraction** on attach is limited (metadata + filename; paste key passages or use Workspace import for text formats)
10. Performance/TalkBack not fully hardware-verified
11. English UI only (i18n scaffolding exists)
12. Primary navigation is 5 tabs (Home / Get / Chat / Mine / Settings); Workspace + Files via Home/Settings
13. RAG / plugins / agents / cloud sync / ads / subscriptions — future only
...
### 1.0.1 — Universal workspace expansion (composer + routing)

- Universal Input Composer: attach files, gallery, camera; attachment chips with preview/remove
- Capability resolver + ChatOrchestrator: intent → chat / summarize / PPT / PDF / sheet / honest image+video gates
- Reliable Send: no silent failures; OOM and missing-model alerts; multi-turn history into the model
- Purpose-first Home tasks: Create Presentation, My Files, clearer Write / Analyze / Voice labels
- Chat → Workspace document generation for presentation/PDF/DOCX/spreadsheet intents
- README honesty updates for attachments, vision, image gen, video

### 1.0.1 — Download progress + chat keyboard

- Live download % / progress bar on Get + model cards; success alert with Open Chat
- Chat composer stays above keyboard on Android
...
App version: **1.0.1** (Android `versionCode` 18).
